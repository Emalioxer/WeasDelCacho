...
public:
  ...
  {
    if (choice < 0 || choice >= (int) actuacionesPosibles.size()) return;
    ...
    if (selected.accion == "lanzar")
    {
      ...
      {
        lanzamiento--;
        if (selected.anotacion.juego == "escalera") marcadores[jugadorActual].escaleraRespetada = true;
        else if (selected.anotacion.juego == "full") marcadores[jugadorActual].fullRespetado = true;
        else if (selected.anotacion.juego == "poker") marcadores[jugadorActual].pokerRespetado = true;
      }
      if (lanzamiento < 2)
      {
        realizarLanzamiento();
  ...
  void anotar(const std::string &jogador, Actuacion act)
  {
        ...
        act.accion == "sobre")
    {
      ...
      if (pos != std::string::npos) juego = juego.substr(0, pos);
      if (juego == "grande" && marcadores[jogador].yaAnotado("grande")) juego = "grande2";
      if (act.accion != "sobre") marcadores[jogador].anotar(juego, act.anotacion.puntos);
    }
  ...
  {
    if (marcadores[jogador].dormida) return true;
    if (marcadores[jogador].tieneAlalay()) return true;
    if (!marcadores[jogador].marcadorIncompleto()) return true;
    return false;
  }
  ...
  int verificarSuma(int valor, const std::vector<int> &dList)
  {
    ...
    if (d[0] == valor && d[4] == valor) return 4 * valor;
    int s = 0, vCount = 0;
    for (int v: dList)
    {
      if (v == valor) s += valor;
  ...
  {
    int s = 0;
    for (int v: dList) if (v == valor) s += valor;
    return s;
  }
                         ...
                         const std::string &jogador)
  {
    for (int i = 0; i < 5; ++i)
    {
      std::vector<int> d = dList;
      d[i] = 7 - d[i];
      if (esInutil(d, jogador)) return true;
      for (int j = i + 1; j < 5; ++j)
      {
        std::vector<int> d2 = d;
        d2[j] = 7 - d2[j];
        if (esInutil(d2, jogador)) return true;
      }
    }
    return false;
  }
  ...
  bool esInutil(const std::vector<int> &d, const std::string &jogador)
  {
    ...
    for (int k = 1; k <= 6; ++k)
    {
      ...
      {
        f++;
        if (verificarSumaSinVuelque(k, d) == 0) c++;
        else return false;
      }
    }
    if (!verificarSiYaAnotado("escalera", jogador))
    {
      f++;
      if (!verificarEscaleraDeMano(d)) c++;
      else return false;
    }
    if (!verificarSiYaAnotado("full", jogador))
    {
      f++;
      if (!verificarFullDeMano(d)) c++;
      else return false;
    }
    if (!verificarSiYaAnotado("poker", jogador))
    {
      f++;
      if (!verificarPokerDeMano(d)) c++;
      else return false;
    }
    if (!verificarSiYaAnotado("grande", jogador))
    {
      f++;
      if (!verificarDormida(d)) c++;
      else return false;
    }
    return c == f;
  }
  ...
  bool verificarEscaleraDeHuevo(const std::vector<int> &dList)
  {
    ...
    if (v3 == 0 || v4 == 0)
    {
      ...
      if ((v1 == 2 || v6 == 2) && v2 == 1) return false;
    }
    return true;
  }
  ...
  bool verificarGrande(const std::vector<int> &dList)
  {
    ...
    if (d[0] == 7 - d[4])
    {
      if (d[0] == d[3] || d[1] == d[4]) return true;
      if ((d[0] == d[2] && d[3] == d[4]) || (d[2] == d[4] && d[0] == d[1])) return true;
    }
    return false;
  }
  ...
  bool ejecutarTurno(const std::string &nome)
  {
    ...
    if (!j) return false; // Should not happen in simulation if bots are added
    ...
    while (!turnoTerminado)
    {
      ...
      std::cout << "\n" << BOLD << "========================================" << RESET << std::endl;
      std::cout << BOLD << "TURNO DE: " << GREEN << j->nombre << RESET << std::endl;
      std::cout << BOLD << "Dados actuales: " << YELLOW;
      for (int d: dados) std::cout << "[" << d << "] ";
      std::cout << RESET << std::endl;
      std::cout << BOLD << "Intento: " << intento << " | Lanzamiento: " << lanzamiento << " | Puntaje: " << marcadores[
        nome].suma << RESET << std::endl;
      std::cout << BOLD << "========================================" << RESET << std::endl;

      // crear espacio para el texto del bot (si es que tiene)
#endif
      ...
      if (!rerollIdx.empty())
      {
        ...
        for (int i: rerollIdx)
        {
          ...
          std::cout << "[" << i << "] " << BLUE << "Relanzar índices: " << RESET
              << "[ ";
          for (int d: a.indiceDados) std::cout << d << " ";
      ...
      if (!scoringIdx.empty())
      {
        std::cout << BOLD << BLUE << "\n--- OPCIONES DE ANOTACIÓN ---" << RESET << std::endl;

        for (int i: scoringIdx)
        {
          const auto &a = actuacionesPosibles[i];
          if (choice == i) std::cout << RED;
          else std::cout << GRAY;

          if (a.accion == "dormida")
          {
            std::cout << "[" << i << "] " << BOLD << "DORMIDA! (Gana el juego)" << RESET << std::endl;
          }
          else if (a.accion == "sobre")
          {
            std::cout << "[" << i << "] " << BOLD
                << "RESERVAR (Sobre): " << RESET;

            if (choice == i) std::cout << RED;
            else std::cout << GRAY;

            std::cout << a.anotacion.juego << " (+" << a.anotacion.puntos << " pts)" << RESET << std::endl;
          }
          else
          {
            bool esTachar = (a.anotacion.puntos == 0);
            std::cout << "[" << i << "] "
                << (esTachar ? "Tachar " : "Anotar ") << BOLD
                << a.anotacion.juego << " (+" << a.anotacion.puntos
                << " pts)" << RESET << std::endl;
          }
        }
      }
      ...
      if (!rerollIdx.empty())
      {
        std::cout << BOLD << YELLOW << "\n--- OPCIONES DE RELANZAMIENTO ---" << RESET << std::endl;
        for (int i: rerollIdx)
        {
          const auto &a = actuacionesPosibles[i];
          ...
          std::cout << "[" << i << "] " << "Relanzar índices: " << "[ ";

          for (int d: a.indiceDados) std::cout << d << " ";
          std::cout << "]" << (a.seRespeta ? " (Respetando)" : "") << RESET << std::endl;
        }
      }

      if (!otherIdx.empty())
      {
        std::cout << BOLD << RED << "\n--- OTRAS OPCIONES ---" << RESET << std::endl;
        for (int i: otherIdx)
        {
          const auto &a = actuacionesPosibles[i];
          if (choice == i) std::cout << RED;
  ...
  std::vector<std::string> hacerJugar()
  {
    ...
    {
      int maxP = -1;
      for (const auto &n: listaJugadores) maxP = std::max(maxP, marcadores[n].suma);
      for (const auto &n: listaJugadores) if (marcadores[n].suma == maxP) ganadores.push_back(n);
    }
    return ganadores;
  }
};
